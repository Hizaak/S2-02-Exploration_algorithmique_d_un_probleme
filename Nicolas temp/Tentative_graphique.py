...
#g.add_weighted_edges_from(['QUIE', 'BELH', 232.57034889681984])
liste_test=[]
compteur={}
for i in nom_arrets:
    if i not in liste_test:
        liste_test.append(i)
        compteur[i]=1
    else:
        compteur[i]+=1
...
for i in range(len(arrets)):
    g.add_node(i)
    for j in arrets[nom(i)][2]:
        g.add_weighted_edges_from([i, indice_som(j), distarc(nom(i),j)])
"""
